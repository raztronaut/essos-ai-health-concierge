...
| Spectrum transport | Railway (worker, no domain) | Built from [deploy/transport.Dockerfile](../../deploy/transport.Dockerfile). Holds the live Spectrum connection + outbound/reminder loops (can't be serverless). Vars `CONVEX_SITE_URL` + `CONVEX_SERVICE_SECRET`, `EVE_BASE_URL` + `ESSOS_TRANSPORT_SECRET`, `SPECTRUM_*`, `ESSOS_GUEST_MODE=1`, `ESSOS_CONCIERGE_HANDLES`. **Resilience:** a stream watchdog ([transport/src/health.ts](../../transport/src/health.ts)) exits non-zero if the Spectrum stream stays down past the stale window so Railway restarts into a fresh connection (a silently-dead consumer never recovers on its own). Set optional `ESSOS_TRANSPORT_HEALTH_PORT` to expose `GET /healthz` (200 healthy / 503 stale) for a Railway healthcheck. A host-local single-instance lock refuses a second consumer (one per Spectrum project). Locally, run the restart-on-crash supervisor with `pnpm transport:imessage:supervised`. |
