...
import { sendToPatientSpace, type SpectrumApp } from "./outbound.js";
...
async function selectPreopEvent(
  ...
  return candidates.find((event) => event.kind === "clinic") ?? candidates[0] ?? null;
}
...
async function selectFastingInstruction(
  ...
  return (
    preop.find((care) => /eat|drink|food|fast|fasting/i.test(`${care.title} ${care.body}`)) ??
    preop[0] ??
    null
  );
}
...
function whenPhrase(startsAt: string | null, now: Date): string {
  if (!startsAt) return "ahead of your procedure";
  const eventMs = new Date(startsAt).getTime();
  if (Number.isNaN(eventMs)) return "ahead of your procedure";
  const diffMs = eventMs - now.getTime();
  if (diffMs < 0) return "ahead of your procedure";
  if (diffMs <= 18 * HOUR_MS) return "for your procedure later today";
  if (diffMs <= 42 * HOUR_MS) return "for your procedure tomorrow";
  const date = new Date(eventMs).toLocaleDateString("en-US", { month: "short", day: "numeric" });
  return `for your procedure on ${date}`;
}

function buildReminder(patient: Patient, event: ItineraryEvent, instruction: CareInstruction): string {
  return (
...
export async function sendDuePreopReminders(
  ...
  for (const patient of patients) {
    ...
    try {
      const delivered = await sendToPatientSpace(app, conversation.space_id, patient.handle, text);
      if (!delivered) {
 ...
 */
export function startReminderLoop(app: SpectrumApp, intervalMs = HOUR_MS): () => void {
  let running = false;
  const tick = (): void => {
    if (running) return;
    running = true;
