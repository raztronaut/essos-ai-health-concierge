...
export interface MessageLoopOptions {
  ...
  resolveAuthor: (space: Space, message: Message, text: string) => ResolvedAuthor | null;
  onResult: (space: Space, message: Message, result: InboundResult) => Promise<void>;
...
export async function runMessageLoop(opts: MessageLoopOptions): Promise<void> {
  for await (const [space, message] of opts.app.messages) {
    ...
    if (!text) {
      debug("transport", "ignored non-text inbound content:", message.content.type);
      continue;
    }
