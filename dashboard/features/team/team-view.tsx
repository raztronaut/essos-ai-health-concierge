  ...
  if (team === undefined) {
    return (
      <div className="space-y-6">
        <PageHeader title="Team" subtitle="Concierge performance and workload." />
        <Card>
          <p className="text-sm text-muted">Loading team metrics…</p>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <Stat label="Avg resolution" value={fmtDuration(team.totals.avgResolutionMs)} />
        <Stat label="Oldest open" value={fmtDuration(team.totals.oldestOpenMs)} />
      </section>

      <Card>
        <h2 className="text-sm font-semibold">By concierge</h2>
        {rows.length === 0 ? (
          <p className="mt-2 text-sm text-muted">No handled escalations yet.</p>
        ) : (
          <div className="mt-3 overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.assignee} className="border-b border-border/60 last:border-0">
      ...
      {team.members.length > 0 ? (
        ...
        <Card>
          <p className="text-sm text-muted">
            No concierge accounts synced yet. With Clerk configured, team members
            appear here as they sign in (and via the org webhook).
          </p>
        </Card>
      )}
    </div>
